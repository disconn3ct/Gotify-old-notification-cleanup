...
while getopts ":hd:a:c:k:gsf:" option; do
	...
	f) # set CentralConfigFile
		...
		[[ -r "$CentralConfigFile" ]] || {
			echo "ERROR - Custom config file is set, but not could not be read under $CentralConfigFile."
			exit 1
		}
		;;
	s)
...
[[ "$connectivityCheck" == "401" ]] && {
	echo "ERROR - Unauthorized. Please check Client Token."
	exit 401
}
[[ "$connectivityCheck" == "000" ]] && {
	echo "ERROR - Host not reacheble under https://$GotifyDomain. Please check if Server and Port are correct."
	exit 500
}
